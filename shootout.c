...
static uint64_t
loopmix128(uint64_t s[3])
{
    uint64_t m = 0x9e3779b97f4a7c15;
    uint64_t r = m * (s[2] + s[1]);
    if (!s[1]) {
        s[0] += m;
        s[2] ^= s[0];
    }
    s[2] = (s[2]<<59 | s[2]>> 5) + s[1];
    s[1] = (s[1]<<47 | s[1]>>17) + m;
    return r;
}
...
#define LOOPMIX128_SETUP() \
    uint64_t state[] = { \
        0xdeadbeefcafebabe, 0x8badf00dbaada555, 0x4cf08ad601831eb6 \
    }
#define LOOPMIX128_RAND(dst) \
    dst = loopmix128(state)
...
DEFINE_BENCH(loopmix128, LOOPMIX128_SETUP, LOOPMIX128_RAND);
...
main(int argc, char **argv)
{
    static const struct {
        ...
        {loopmix128_bench,       loopmix128_pump,       "loopmix128"},
    };
