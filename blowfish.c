...
void
blowfish_encrypt16(struct blowfish *ctx, uint32_t *bl, uint32_t *br)
{
    uint32_t xl = *bl;
    uint32_t xr = *br;
...
void
blowfish_encrypt4(struct blowfish *ctx, uint32_t *bl, uint32_t *br)
{
    uint32_t xl = *bl;
    uint32_t xr = *br;
    for (int i = 0; i < 4; i += 2) {
...
static void
blowfish_expand(struct blowfish *ctx, const void *key, int len)
{
    const uint8_t *k = key;
    for (int i = 0; i < 18; i++) {
        /* big endian */
    ...
    for (int i = 0; i < 18; i += 2) {
        blowfish_encrypt16(ctx, ctext, ctext + 1);
        ctx->p[i + 0] = ctext[0];
        ctx->p[i + 1] = ctext[1];
    }
